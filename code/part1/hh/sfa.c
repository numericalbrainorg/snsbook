...
#define TAU_AHP (  200 ) // ms
#define G_AHP   ( 1400 ) // mS / cm^2
...
static inline double dadt ( const int s,    const double a ) { return ( 1.0 / TAU_AHP ) * ( - a + s ); }
static inline double dvdt ( const double v, const double m, const double h, const double n, const double a, const double i_ext )
{
...
int main ( void )
{
  ...
  for ( int32_t nt = 0; nt < NT; nt++ ) {
    ...
    double dadt2 = dadt ( s,                   a + .5 * DT * dadt1 );
    double dvdt2 = dvdt ( v + .5 * DT * dvdt1, m + .5 * DT * dmdt1, h + .5 * DT * dhdt1, n + .5 * DT * dndt1, a + .5 * DT * dadt1, i_ext ); 
    ...
    double dadt3 = dadt ( s,                   a + .5 * DT * dadt2 );
    double dvdt3 = dvdt ( v + .5 * DT * dvdt2, m + .5 * DT * dmdt2, h + .5 * DT * dhdt2, n + .5 * DT * dndt2, a + .5 * DT * dadt2, i_ext ); 
    ...
    double dvdt4 = dvdt ( v + DT * dvdt3, m + DT * dmdt3, h + DT * dhdt3, n + DT * dndt3, a + DT * dadt3, i_ext ); 
