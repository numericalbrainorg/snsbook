...
int main ( void )
{
  ...
  for ( int32_t nt = 0; nt < NT; nt++ ) {
    ...
    for ( int32_t i = 0; i < N; i++ ) { i_ahp [ i ] = exp ( - DT / TAU_AHP ) * i_ahp [ i ] + s [ i ]; }
    for ( int32_t i = 0; i < N; i++ ) {
      v [ i ] += DT * ( - ( v [ i ] - V_REST ) + R_SYN * i_syn [ i ] - R_AHP * i_ahp [ i ] + R_M * I_EXT ) / TAU;
      s [ i ] = ( v [ i ] > THETA );
    }
